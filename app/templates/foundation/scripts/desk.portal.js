...
(function (a) {
  "use strict", a.extend({ placeholder: { settings: { focusClass: "placeholderFocus", activeClass: "placeholder", overrideSupport: !1, preventRefreshIssues: !0 } } }), a.support.placeholder = "placeholder" in document.createElement("input"), a.fn.plVal = a.fn.val, a.fn.val = function (b) {
    if (typeof b == "undefined") return a.fn.plVal.call(this);var c = a(this[0]),
        d = c.plVal(),
        e = a(this).plVal(b);return c.hasClass(a.placeholder.settings.activeClass) && d === c.attr("placeholder") ? (c.removeClass(a.placeholder.settings.activeClass), e) : c.hasClass(a.placeholder.settings.activeClass) && c.plVal() === c.attr("placeholder") ? "" : a.fn.plVal.call(this, b);
  }, a(window).bind("beforeunload.placeholder", function () {
    var b = a("input." + a.placeholder.settings.activeClass);b.length > 0 && b.val("").attr("autocomplete", "off");
    ...
    return b = a.extend({}, a.placeholder.settings, b), !b.overrideSupport && a.support.placeholder ? this : this.each(function () {
      var c = a(this);if (!c.is("[placeholder]")) return;if (c.is(":password")) return;b.preventRefreshIssues && c.attr("autocomplete", "off"), c.bind("focus.placeholder", function () {
        var c = a(this);this.value === c.attr("placeholder") && c.hasClass(b.activeClass) && c.val("").removeClass(b.activeClass).addClass(b.focusClass);
      }), c.bind("blur.placeholder", function () {
        var c = a(this);c.removeClass(b.focusClass), this.value === "" && c.val(c.attr("placeholder")).addClass(b.activeClass);
...
if (currentPage == 'chat_pre_create') {
  ...
  if (deskEV('number_search_results') == '0') {
    var _openChatWindow = function _openChatWindow() {
      window.open('', 'newChatWin', 'width=500,height=500,toolbar=0');
    };
    ...
    jQuery(document).ready(function () {
      $('#chat_submit').click(function () {
        $(this).addClass('disabled');
        setTimeout(function () {
          window.location = $('#breadcrumbs ul.breadcrumbs li:first-child a').attr("href");
        }, 5000);
      });
    });
  } else {
    var _openChatWindow2 = function _openChatWindow2() {
      window.open('', 'newChatWin', 'width=500,height=500,toolbar=0');
    };
...
if (currentPage == 'email_new' || currentPage == 'question_new' || currentPage == 'email_pre_create' || currentPage == 'chat_new') {
  (function () {
    ...
    var articleSuggestMultiBrand = function articleSuggestMultiBrand() {
    ...
    var articleSuggest = function articleSuggest() {
      var search_query = '',
    ...
    var listArticles = function listArticles(data) {
      var as_count = 0,
          article_title = "",
    ...
    var apiFail = function apiFail(data) {};
  ...
  (function ($) {
    if ($('#MyCaseSort').val() != null) {
      ...
      if ($('#pagination a.next_page').length) {
        ...
        (function () {
          var nextPage = function nextPage(url, callback) {
          ...
          nextUrl = $('#pagination a.next_page');
...
if (currentPage == 'myaccount') {
  (function () {
    var check_duplicate = function check_duplicate() {
      var return_val = true;
...
if (currentPage == 'customer_feedback') {
  ...
  $(function () {
    $('.btn-radio').click(function (e) {
      $('.btn-radio').not(this).removeClass('active').siblings('input').prop('checked', false);
...
$(document).ready(function () {
  ...
  $("#a-content-select").change(function (event) {
    ...
    if (r && r.length > 1) location.href = "/customer/" + $(this).val() + "/portal" + r[2] + location.search;else location.href = "/customer/" + $(this).val() + "/portal/articles" + location.search;
  });
  // =====================================================
  // Mobile Related
  // =====================================================
    ...
    success: function success(data) {
      ...
      function apiSuccess(data) {
        ...
        if (as_count > 0) {
          ...
          if (as_count > 9) {
            $('#SearchAutoSuggest').append(' ' + '<div class="columns medium-12" id="brand-results">' + '<div class="row"><h5 class="left subheader"><span>' + as_count + ' + </span>' + resultsFound + ' in Site Name</h5>' + '<a class="button success tiny right" target="_blank" href="//' + document.location.host.toString() + '/customer/' + systemLanguageDesk + '/portal/articles/search?q=' + search_query + '">View All</a></div>' + '<div class="row"><ul class="unstyled"></ul></div>');
          } else {
            $('#SearchAutoSuggest').append('<div class="columns medium-12" id="brand-results">' + '<div class="row"><h5 class="left subheader"><span>' + as_count + ' </span>' + resultsFound + ' in Site Name</h5>' + '<a class="button success tiny right " target="_blank" href="//' + document.location.host.toString() + '/customer/' + systemLanguageDesk + '/portal/articles/search?q=' + search_query + '">View All</a></div>' + '<div class="row"><ul class="unstyled"></ul></div>');
          }
          if (as_count > 0) {
      ...
      success: function success(data) {
        ...
        function apiSuccess(data, brandID, brandName) {
          ...
          if (as_count > 0) {
            $('#SearchAutoSuggest').show();
            if (as_count > 9) {
              $('#SearchAutoSuggest').append(' ' + '<div class="columns medium-12" id="brand-' + brandID + '">' + '<div class="row"><h4 class="left subheader"><span>' + as_count + ' + </span>' + resultsFound + ' in ' + brandName + '</h4>' + '<a class="button tiny right" target="_blank" href="//' + document.location.host.toString() + '/customer/' + systemLanguageDesk + '/portal/articles/search?b_id=' + brandID + '&q=' + search_query + '">View All</a></div>' + '<div class="row"><ul class="unstyled"></ul></div>');
            } else {
              $('#SearchAutoSuggest').append('<div class="columns medium-12" id="brand-' + brandID + '">' + '<div class="row"><h4 class="left subheader"><span>' + as_count + ' </span>' + resultsFound + ' in ' + brandName + '</h4>' + '<a class="button tiny right" target="_blank" href="//' + document.location.host.toString() + '/customer/' + systemLanguageDesk + '/portal/articles/search?b_id=' + brandID + '&q=' + search_query + '">View All</a></div>' + '<div class="row"><ul class="unstyled"></ul></div>');
            }
            if (as_count > 0) {
...
//# sourceMappingURL=desk.portal.js.map
